...
Output files generated by the model simulations:
    ...
    (2) dataOutput folder: Stores all txt files with a "detailedStat_'' prefix. These files include information about every 
        cell such as its rank, number of neighbors, cell area, cell center, cell pressure and the position of the apical node, 
        basal node and nucleus center. 
            * For an example, see the provided sample file detailedStat_example.txt.
    (3) ECMFolder: Includes txt files with information about the ECM nodes along with the vtk files for visualizing the ECM nodes.
    (4) animation folder: Includes vtk files for viewing simulation results. These files store the information for visualizing the 
        nuclei and membrane nodes.
