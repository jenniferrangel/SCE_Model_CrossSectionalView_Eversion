...
void SceNodes::sceForcesDisc_M(double timeRatio, double timeRatio_Crit_Division, int cycle,
	...
	if (timeRatio == 0 && cycle <= 0){
		...
		//The following code assigns the apical and basal contractility where the tissue is split up into 3 parts, a medial and 2 lateral domains, so that it facilitates testing higher contractility in the center
		//The contractActomyo_multip_perCell# and contractActomyo_multip_perCell_apical# are assiged on discMain_M.cpp
