...
struct AddLagrangeForces: public thrust::unary_function<DUiDDUiUiDDDD, CVec5> {
	...
	__device__ CVec5 operator()(const DUiDDUiUiDDDD &dUiDDUiUiDDDD) const {
		...
		//		cellAreaDesire=65+ percent*65 ;
				cellAreaDesire = 45;                //Target cell volume for cells 0 and 64 of the columnar cells (start and end of columnar epithelia). This is for linear gradient targetVolume = 65 - k|CellID - 32| where k = 0.625
			}
			else if (cellRank==1 || cellRank==63) {
				//cellAreaDesire=50+ percent*50 ;
				cellAreaDesire = 45.625;           //Target cell volume for cells 1 and 63. This is for linear gradient targetVolume = 65 - k|CellID - 32| where k = 0.625
			}
			//ChangesMadeByJRA_end
			...
			// }
				...
				//Now assign the target volume for the columnar cells using equation targetVolume = 65 - k|CellID - 32| where k = 0.625
				else if(cellRank >= 2 && cellRank < 33){
					cellIdDiff = 32 - cellRank;
					cellAreaDesire = 65 - (0.625*cellIdDiff);
				}
				else if(cellRank >= 33 && cellRank <= 62){
					cellIdDiff = cellRank - 32;
					cellAreaDesire = 65 - (0.625*cellIdDiff);
				}
