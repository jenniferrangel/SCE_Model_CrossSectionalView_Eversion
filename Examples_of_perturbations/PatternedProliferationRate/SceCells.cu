...
//	computeCenterPos_M();    //Ali cmment //
	...
	//ChangesMadeByKT_begin
	//Here I am setting the initial columnar cells into three section. The current setup is that 0: anterior, 1: medial, 2: posterior
...
void SceCells::copyFirstCellArr_M(double quiescence1, double quiescence1_half) {
	...
	for (uint i = 0; i < divAuxData.toBeDivideCount; i++) {
		...
		//This patterns the proliferation rate in the tissue such that the proliferation rate in the medial region(1) is different than that of the lateral ends (0 and 2)
...
void SceCells::divide2D_M(double volume_Increase_Target_Ratio, double timeRatio, double thresholdToIntroduceNewCell) {
	...
	rng.discard(allocPara_m.currentActiveCellCount);
		...
		thrust::uniform_real_distribution<double> distribution_half(0.0, 1.0);  //Changing this changes proliferation in which ever section (medial or lateral) where it is defined
				//Examples on how to change proliferation rate:
				//distribution_half(0.0, 0.14) ---> 1.4 fold increase in proliferation rate 
				//distribution_half(0.10, 0.40) ---> 2 fold increase in proliferation rate 
				//distribution_half(0.8, 0.9) ---> 10 fold increase in proliferation rate 
				//distribution_half(0.90, 0.95) ---> 20 fold increase in proliferation rate 
		//ChangesMadeByJRA_begin
