	...
	__device__ CVec10 operator()(const TensionData_type2 &tData2) const {
		...
		if (_isActiveAddr[index] == false || nodeRank >= activeMembrCount) {
			...
			if (_isActiveAddr[index_left]) {
				...
				//For each cell in its respective domain, the membrane has been split into lateralA, lateralB, apical1 and basal1 sides allowing us to change the spring coefficient in E_memb based upon if the spring coefficient belongs to the apical, basal or lateral sides of the cell
				if (_cellSubdomainIndx[cellRank] == 0){
					if (memType == lateralA ){
						forceVal = forceVal;         //Ex1: To decrease/increase the lateral tension by 50% in the lateral domains then this line becomes forceVal = forceVal*0.5 (to decrease) or forceVal = forceVal*1.5 (to decrease)  
					}
					else if (memType == lateralB){
						forceVal = forceVal;         //Ex1: To decrease/increase the lateral tension by 50% in the lateral domains then this line becomes forceVal = forceVal*0.5 (to decrease) or forceVal = forceVal*1.5 (to decrease)
					}
					else if (memType == apical1){
						forceVal = forceVal;        //modify this line if we want to inc/dec the apical membrane tension
					}
					...
					if (memType == lateralA ){
						...
						forceVal = forceVal;        //Ex2: To increase the apical membrane tension by 50% in the medial region we can change this line to forceVal = forceVal*1.5 
					}
					else if (memType == basal1){
						forceVal = forceVal;        //Ex3: To increase the basak membrane tension by 50% in the medial region we can change this line to forceVal = forceVal*1.5 
					}
				}
			...
			if (_isActiveAddr[index_right]) {
				...
				//ChangesMadeByKTandJRA_begin
				//As before we have split the tissue into 3 sections to increase or decrease the apical,basal and lateral membrane tension in their respective region (lateral or medial)
				//Whatever changes made to the code above (associated to index_left) also have to be applied here
				if (_cellSubdomainIndx[cellRank] == 0){
					if (memType == lateralA ){
						forceVal = forceVal;
					}
...
struct CalMembrEnergy: public thrust::unary_function<DUiUiUiDD, CVec2> {
	...
	__device__ CVec2 operator()(const DUiUiUiDDT & dUiUiUiDDT) const {
		...
		if (_isActiveAddr[index] == false || nodeRank >= activeMembrCount) {
	        ...
	        //The current setup is that 0: anterior, 1: medial, 2: posterior such that 0 and 2 are the 2 lateral domains.
			//For each cell in its respective domain, the membrane has been split into lateralA, lateralB, apical1 and basal1 sides allowing us to change the spring coefficient in E_memb based upon if the spring coefficient belongs to the apical, basal or lateral sides of the cell
			//NOTE: The changes made in struct AddMembrForce to forceVal also have to be made here
				if (_cellSubdomainIndx[cellRank] == 0){
					if (memType == lateralA ){
					...
					if (memType == lateralA ){
						...
						energyLinSpringLeft = energyLinSpringLeft;  //Ex3: To increase the basal membrane tension by 50% in the medial region we can change this line to energyLinSpringLeft = 1.5*energyLinSpringLeft and in the struct AddMembrForce we should have forceVal = forceVal*1.5 
					}
				}
				else if (_cellSubdomainIndx[cellRank] == 2){
					if (memType == lateralA ){
			...
			//ChangesMadeByKTandJRA_begin
			//The changes made above associated to index_left should also be made below
				if (_cellSubdomainIndx[cellRank] == 0){
					if (memType == lateralA ){
