...
//use the following function to increase or decrease the spring equilibrium length of either lateral or center regions when the lateral or basal tension needs to be patterned
__device__
	...
	__device__ CVec10 operator()(const TensionData_type2 &tData2) const {
		...
		if (_isActiveAddr[index] == false || nodeRank >= activeMembrCount) {
			...
			if (_isActiveAddr[index_left]) {
				...
				//Recall that the initial columnar cells are split into 3 sections such that tissue is composed of a medial domain and 2 lateral domains.
	            //The current setup is that 0: anterior, 1: medial, 2: posterior such that 0 and 2 are the 2 lateral domains.
				//For each cell in its respective domain, the membrane has been split into lateralA, lateralB, apical1 and basal1 sides allowing us to change the equilibrium length in E_memb based upon if it belongs to the apical, basal or lateral sides of the cell
				//If we want to pattern the lateral or basal tension by changing the equilibrium according to the cell subdomain index it is in, then uncomment the following piece of code:
				//In particular, use calMembrForce_Actin_changeEquilLen to change the equilibrium length in lateralA, laterlaB, apical1 or basal1 in lateral(0 and 2) or medial(1) domains
				double forceVal;
				if (_cellSubdomainIndx[cellRank] == 0){
					if (memType == lateralA ){
						forceVal =calMembrForce_Actin(lenLeft,kAvgLeft);
					}
					...
					if (memType == lateralA ){
						forceVal =calMembrForce_Actin_changeEquilLen(lenLeft,kAvgLeft);  //we use calMembrForce_Actin_changeEquilLen to perturb the lateral membrane tension in the medial domain. In this particular case, we are increasing it by 50%
					}
					else if (memType == lateralB){
						forceVal =calMembrForce_Actin_changeEquilLen(lenLeft,kAvgLeft); ////we use calMembrForce_Actin_changeEquilLen to perturb the lateral membrane tension in the medial domain. In this particular case, we are increasing it by 50%
					}
				...
				//ChangesMadeByJRA_end
				//ChangesMadeByKT_begin
				//If we want to pattern the lateral or basal tension by changing the linear spring coefficient according to the cell subdomain index it is in uncomment following code
				//we multiply forceVal by a value for ex forceVal*0.5 to decrease it by 50%. Otherwise, if we do not want to change it just leave it as forceVal for all 3 domains
				/* if (_cellSubdomainIndx[cellRank] == 0){
					if (memType == lateralA ){
						forceVal = 0.5*forceVal;
					}
					else if (memType == lateralB){
						forceVal = 0.5*forceVal;
					}
			...
			if (_isActiveAddr[index_right]) {
				...
				if (_cellSubdomainIndx[cellRank] == 0){
					if (memType == lateralA ){
						forceVal =calMembrForce_Actin(lenRight,kAvgRight);
					}
					else if (memType == lateralB){
						forceVal =calMembrForce_Actin(lenRight,kAvgRight);
					}
					...
					if (memType == lateralA ){
						forceVal =calMembrForce_Actin_changeEquilLen(lenRight,kAvgRight); //we use calMembrForce_Actin_changeEquilLen to perturb the lateral membrane tension in the medial domain. In this particular case, we are increasing it by 50%
					}
					else if (memType == lateralB){
						forceVal =calMembrForce_Actin_changeEquilLen(lenRight,kAvgRight); //we use calMembrForce_Actin_changeEquilLen to perturb the lateral membrane tension in the medial domain. In this particular case, we are increasing it by 50%
					}
				...
				//ChangesMadeByJRA_end
				//ChangesMadeByKT_begin
				//If we want to pattern the lateral or basal tension by changing the linear spring coefficient according to the cell subdomain index it is in
				//we multiply forceVal by a value for ex forceVal*0.5 to decrease it by 50%. Otherwise, if we do not want to change it just leave it as forceVal for all 3 domains
				/* if (_cellSubdomainIndx[cellRank] == 0){
					if (memType == lateralA ){
						forceVal = 0.5*forceVal;
...
struct CalMembrEnergy: public thrust::unary_function<DUiUiUiDD, CVec2> {
	...
	__device__ CVec2 operator()(const DUiUiUiDDT & dUiUiUiDDT) const {
		...
		if (_isActiveAddr[index] == false || nodeRank >= activeMembrCount) {
			...
			//For each cell in its respective domain, the membrane has been split into lateralA, lateralB, apical1 and basal1 sides allowing us to change the equilibrium length in E_memb based upon if it belongs to the apical, basal or lateral sides of the cell
			//If we want to pattern the lateral or basal tension by changing the equilibrium according to the cell subdomain index it is in, then uncomment the following piece of code:
			//In particular, use CalMembrLinSpringEnergy_changeEquilLen to change the equilibrium length in lateralA, laterlaB, apical1 or basal1 in lateral(0 and 2) or medial(1) domains
			double energyLinSpringLeft;
			if (_cellSubdomainIndx[cellRank] == 0){
					if (memType == lateralA ){
						energyLinSpringLeft = CalMembrLinSpringEnergy(lenLeft,kAvgLeft);
					}
					else if (memType == lateralB){
						energyLinSpringLeft = CalMembrLinSpringEnergy(lenLeft,kAvgLeft);
					}
					...
					if (memType == lateralA ){
						energyLinSpringLeft = CalMembrLinSpringEnergy_changeEquilLen(lenLeft,kAvgLeft); //we use CalMembrLinSpringEnergy_changeEquilLen to perturb the lateral membrane tension in the medial domain. In this particular case, we are increasing it by 50%
					}
					else if (memType == lateralB){
						energyLinSpringLeft = CalMembrLinSpringEnergy_changeEquilLen(lenLeft,kAvgLeft); //we use CalMembrLinSpringEnergy_changeEquilLen to perturb the lateral membrane tension in the medial domain. In this particular case, we are increasing it by 50%
					}
			...
			//ChangesMadeByKT_begin
			//If we want to pattern the lateral or basal tension by changing the linear spring coefficient according to the cell subdomain index it is in uncomment following code
			//we multiply energyLinSpringLeft by a value for ex energyLinSpringLeft*0.5 to decrease it by 50%. Otherwise, if we do not want to change it just leave it as energyLinSpringLeft for all 3 domains
				/* if (_cellSubdomainIndx[cellRank] == 0){
					if (memType == lateralA ){
			...
			//If we want to pattern the lateral or basal tension by changing the equilibrium according to the cell subdomain index it is in, then uncomment the following piece of code:
			//In particular, use CalMembrLinSpringEnergy_changeEquilLen to change the equilibrium length in lateralA, laterlaB, apical1 or basal1 in lateral(0 and 2) or medial(1) domains
			double energyLinSpringRight;
			if (_cellSubdomainIndx[cellRank] == 0){
					if (memType == lateralA ){
						energyLinSpringLeft = CalMembrLinSpringEnergy(lenRight,kAvgRight);
					}
					else if (memType == lateralB){
						energyLinSpringLeft = CalMembrLinSpringEnergy(lenRight,kAvgRight);
					}
					...
					if (memType == lateralA ){
						energyLinSpringLeft = CalMembrLinSpringEnergy_changeEquilLen(lenRight,kAvgRight); //we use CalMembrLinSpringEnergy_changeEquilLen to perturb the lateral membrane tension in the medial domain. In this particular case, we are increasing it by 50%
					}
					else if (memType == lateralB){
						energyLinSpringLeft = CalMembrLinSpringEnergy_changeEquilLen(lenRight,kAvgRight); //we use CalMembrLinSpringEnergy_changeEquilLen to perturb the lateral membrane tension in the medial domain. In this particular case, we are increasing it by 50%
					}
			...
			//ChangesMadeByJRA_end
			//ChangesMadeByKT_begin
			//If we want to change the linear spring coeff, we multiply energyLinSpringLeft by a value for ex energyLinSpringLeft*0.5 to decrease it by 50%. Otherwise, if we do not want to change it just leave it as energyLinSpringLeft for all 3 domains
				/* if (_cellSubdomainIndx[cellRank] == 0){
					if (memType == lateralA ){
