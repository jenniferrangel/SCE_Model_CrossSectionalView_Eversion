...
//use the following function to increase or decrease the spring equilibrium length of either lateral or center regions when the lateral,apical or basal tension needs to be patterned
__device__
double calMembrForce_Actin_changeEquilLen(double& length, double kAvg) {
			return ((length - 0.5*membrEquLen) * kAvg);   //0.5*membrEquLen reduces the membrane equilibrium length by 50%. The equilibrium length can be inc or dec by multiplying by a number bigger or less than 1, respectively 
}
...
//ChangesMadeByJRA_begin
//use the following function to increase or decrease the spring equilibrium length of either lateral, apical or center regions when the lateral or basal tension needs to be patterned
__device__
double CalMembrLinSpringEnergy_changeEquilLen(double& length, double kAvg) {
			return  (0.5*kAvg *(length - 0.5*membrEquLen)*(length - 0.5*membrEquLen)) ; //0.5*membrEquLen reduces the membrane equilibrium length by 50%. The equilibrium length can be inc or dec by multiplying by a number bigger or less than 1, respectively
}
//ChangesMadeByJRA_end
...
//	computeCenterPos_M();    //Ali cmment //
	...
	if (1 < 0){   //Changes: to turn off proliferation uncomment this line and comment out the next line
...
//	}
	...
	thrust::transform(
			...
			AddMembrForce(allocPara_m.bdryNodeCount, maxAllNodePerCell,
					...
					nodeIsActiveAddr, nodeAdhereIndexAddr,
					nodeActinLevelAddr, grthPrgrCriVal_M,
					minY_Cell,maxY_Cell,
					cellSubdomainIndx));
			//ChangesMadeByKT_end
...
thrust::transform(
			...
			CalMembrEnergy(maxAllNodePerCell,nodeLocXAddr, 
