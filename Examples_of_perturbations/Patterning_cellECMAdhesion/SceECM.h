	...
	double scaling;
		...
		if ( nodeIsActive && nodeRankInOneCell<_maxMembrNodePerCell ) {
			...
			if (IsValidAdhPairForNotInitPhase(distMin)&& iPair!=-1) {
				...
				//We can increase cell-ECM adhesion by 50% in lateral ends by multiplying the the spring constant of the lateral ends by 1.5
				if (_cellSubdomainIndx[cellRank] == 0){
					fAdhMemECM = 1.5*fAdhMemECM;   //we can also decrease the cell-ECM adhesion by multiplying by a number less than 1
				}
				else if (_cellSubdomainIndx[cellRank] == 1){  //we can also increase or decrease the cell-ECM adhesion by multiplying fAdhMemECM by a number greater than or less than 1, respectively 
					fAdhMemECM = fAdhMemECM;
				}
				else if (_cellSubdomainIndx[cellRank] == 2){
					fAdhMemECM = 1.5*fAdhMemECM;
				}
				eAdhCell=CalAdhEnergy(distMin) ; 
				if (_cellSubdomainIndx[cellRank] == 0){
					eAdhCell = 1.5*eAdhCell;
				}
