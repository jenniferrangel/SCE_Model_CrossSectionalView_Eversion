...
struct LinSpringForceECM: public thrust::unary_function<IDD,DDDD> {
	...
	// if (activePairLeft == true){
		...
		//	forceLeft=calWLC_ECM(distLeft) ; 
			...
			//Recall that the tissue is split into an anterior(0) and posterior (1) side
			if (_cellSubdomainIndx[neighborCellIndx] == 0){
				forceLeft = forceLeft;
				energyLeft = energyLeft;
			}
			else if (_cellSubdomainIndx[neighborCellIndx] == 1){
					forceLeft = 1.50*forceLeft;                   //This increases the ECM stiffness by 50%. If we set it to .50*forceLeft, it will lead to a decrease in ECM stiffness
					energyLeft = 1.50*energyLeft;                 //This increases the ECM stiffness by 50%. If we set it to .50*forceLeft, it will lead to a decrease in ECM stiffness
			}
	...
	// if (activePairRight==true){
		...
		if (_cellSubdomainIndx[neighborCellIndx] == 0){
				...
				forceRight = 1.50*forceRight;                  //This increases the ECM stiffness by 50%. If we set it to .50*forceRight, it will lead to a decrease in ECM stiffness
				energyRight = 1.50*energyRight;                //This increases the ECM stiffness by 50%. If we set it to .50*forceRight, it will lead to a decrease in ECM stiffness
		}
