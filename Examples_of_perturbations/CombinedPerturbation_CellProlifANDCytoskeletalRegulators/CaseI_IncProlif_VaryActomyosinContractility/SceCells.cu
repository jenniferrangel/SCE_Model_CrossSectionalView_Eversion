...
//	computeCenterPos_M();    //Ali cmment //
	...
	//ChangesMadeByKT&JRA_begin
	//Here I am setting the initial columnar cells into two sections. The current setup is that 0: anterior, 1: posterior
...
void SceCells::copyFirstCellArr_M(double quiescence1, double quiescence1_half) {
	...
	for (uint i = 0; i < divAuxData.toBeDivideCount; i++) {
		...
		if (cellInfoVecs.cellSubdomainIndx[cellRank] == 0){
			cellInfoVecs.growthProgress[cellRank] = quiescence1;         //anterior
		}
		else if (cellInfoVecs.cellSubdomainIndx[cellRank] == 1){
			cellInfoVecs.growthProgress[cellRank] = quiescence1_half;    //posterior
		}
		 //ChangesMadeByKT&JRA_end
...
void SceCells::copySecondCellArr_M(double quiescence2, double quiescence2_half) {
	...
	for (uint i = 0; i < divAuxData.toBeDivideCount; i++) {
		...
		if (cellInfoVecs.cellSubdomainIndx[cellRankMother] == 0){
			cellInfoVecs.growthProgress[cellRank] = quiescence2;         //anterior
		}
		else if (cellInfoVecs.cellSubdomainIndx[cellRankMother] == 1){
			cellInfoVecs.growthProgress[cellRank] = quiescence2_half;    //posterior
		}
		 //ChangesMadeByKT&JRA_end
...
void SceCells::divide2D_M(double volume_Increase_Target_Ratio, double timeRatio, double thresholdToIntroduceNewCell) {
	...
	rng.discard(allocPara_m.currentActiveCellCount);
		...
		thrust::uniform_real_distribution<double> distribution_half(0.0, 0.5);  //This will increase proliferation on posterior side by 50%
		//ChangesMadeByJRA_begin
	...
	if (isNewCellIntroduced < thresholdToIntroduceNewCell){
		...
		//ChangesMadeByKT&JRA_begin
		if (cellInfoVecs.cellSubdomainIndx[cellRank] == 0){
			cellInfoVecs.growthProgress[cellRank] = quiescence3;      //anterior
		}
		else if (cellInfoVecs.cellSubdomainIndx[cellRank] == 1){
			cellInfoVecs.growthProgress[cellRank] = quiescence3_half; //posterior
		}
		//ChangesMadeByKT&JRA_begin
