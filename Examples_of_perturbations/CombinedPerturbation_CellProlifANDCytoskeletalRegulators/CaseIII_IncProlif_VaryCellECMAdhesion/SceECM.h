		...
		if ( nodeIsActive && nodeRankInOneCell<_maxMembrNodePerCell ) {
			...
			if (IsValidAdhPairForNotInitPhase(distMin)&& iPair!=-1) {
				...
				if (_cellSubdomainIndx[cellRank] == 0){
					...
					fAdhMemECM = 2.0*fAdhMemECM;              //This increases the cell-ECM adhesion by 100%. If we multiply by a number < 1, it will decrease the adhesion
				}
				...
				if (_cellSubdomainIndx[cellRank] == 0){
					...
					eAdhCell = 2.0*eAdhCell;                 //This increases the cell-ECM adhesion by 100%. If we multiply by a number < 1, it will decrease the adhesion
				}
		...
		if (_nodeIsActive_Cell[i] && (i%_maxNodePerCell)<_maxMembrNodePerCell){
			...
			if (_adhPairECM_Cell[i]==index) {
				...
				if (_cellSubdomainIndx[cellId] == 0){
					...
					fAdh = 2.0*fAdh;                         //This increases the cell-ECM adhesion by 100% on posterior side. If we multiply by a number < 1, it will decrease the adhesion
					eAdh = 2.0*eAdh;                         //This increases the cell-ECM adhesion by 100% on posterior side. If we multiply by a number < 1, it will decrease the adhesion
				}
