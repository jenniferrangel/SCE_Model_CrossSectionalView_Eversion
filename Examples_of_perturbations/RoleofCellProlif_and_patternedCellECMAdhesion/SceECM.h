...
struct MoveNodes2_Cell: public thrust::unary_function<IIIDDBT,DDIDD> {
	...
	//  bool* _isActiveECM;
	__host__ __device__ MoveNodes2_Cell (double * locXAddr_ECM, double * locYAddr_ECM, 
							uint maxMembrNodePerCell, int numNodes_ECM, 
							double dt, double Damp_Coef,
							EType * peripORexcmAddr, int activeCellCount, 
							double* cellGrowthProgress, double mitoticThreshold,
							int* cellSubdomainIndx/*, bool* isActiveECM*/) :
			    ...
			    _Damp_Coef(Damp_Coef), _peripORexcmAddr(peripORexcmAddr), _activeCellCount (activeCellCount), _cellGrowthProgress(cellGrowthProgress), _mitoticThreshold(mitoticThreshold),
				_cellSubdomainIndx(cellSubdomainIndx)/*, _isActiveECM(isActiveECM)*/	{
	}
	...
	double scaling;
		...
		if ( nodeIsActive && nodeRankInOneCell<_maxMembrNodePerCell ) {
			...
			if (IsValidAdhPairForNotInitPhase(distMin)&& iPair!=-1) {
				...
				//Recall that the tissue is split into 3 parts a medial region(1) and 2 lateral ends (0 and 2)
				//The following set of code is set up to increase the columnar cell-ECM adhesion by 50% in the lateral ends
				//In general we can increase or decrease cell-ECM adhesion in the medial region(1) or lateral ends (0 and 2) by setting
					//* fAdhMemECM = #*fAdhMemECM;   where # can be > 1 to inc adhesion or < 1 to decrease adhesion
					//* eAdhCell = #*eAdhCell;       where # can be > 1 to inc adhesion or < 1 to decrease adhesion
				if (_cellSubdomainIndx[cellRank] == 0){
					fAdhMemECM = 1.5*fAdhMemECM;                 //Multiplying by 1.5 increases the columnar cell-ECM adhesion by 50%
				}
				else if (_cellSubdomainIndx[cellRank] == 1){
					fAdhMemECM = fAdhMemECM;
				}
				...
				if (_cellSubdomainIndx[cellRank] == 0){
					eAdhCell = 1.5*eAdhCell;                    //Multiplying by 1.5 increases the columnar cell-ECM adhesion by 50%
				}
				else if (_cellSubdomainIndx[cellRank] == 1){
					eAdhCell = eAdhCell;
				}
...
//	}
 ...
 struct MorseAndAdhForceECM: public thrust::unary_function<IDDI,DDDD> {
	...
	_numCells(numCells), _maxNodePerCell(maxNodePerCell), _maxMembrNodePerCell(maxMembrNodePerCell),_locXAddr_Cell(locXAddr_Cell),_locYAddr_Cell(locYAddr_Cell),_nodeIsActive_Cell(nodeIsActive_Cell),_adhPairECM_Cell(adhPairECM_Cell), /*_isActiveECM(isActiveECM),*/ _nodeCellGrowProAddr(nodeCellGrowProAddr), _mitoticThreshold(mitoticThreshold),
								_cellSubdomainIndx(cellSubdomainIndx) {
	}
	 __device__ 
    ...
    //for (int i=0 ; i<_numCells*_maxNodePerCell ; i++) {
		...
		if (_nodeIsActive_Cell[i] && (i%_maxNodePerCell)<_maxMembrNodePerCell){
			...
			if (_adhPairECM_Cell[i]==index) {
				...
				if (_cellSubdomainIndx[cellId] == 0){
					fAdh = 1.5*fAdh;
					eAdh = 1.5*eAdh;
				}
				else if (_cellSubdomainIndx[cellId] == 1){
					fAdh = fAdh;
					eAdh = eAdh;
				}
				else if (_cellSubdomainIndx[cellId] == 2){
					fAdh = 1.5*fAdh;
					eAdh = 1.5*eAdh;
				}
