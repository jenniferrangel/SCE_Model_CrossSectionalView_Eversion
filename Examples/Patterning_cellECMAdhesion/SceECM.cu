...
//// find the closest ECM node to each each cell //
 ...
 int* cellSubdomainIndx = thrust::raw_pointer_cast(
			&cellsPointerECM->getCellInfoVecs().cellSubdomainIndx[0] );
 ...
 thrust:: transform (
		...
		thrust::make_zip_iterator (
				thrust::make_tuple (
					...
					adhEnergyCell.begin())),
				MoveNodes2_Cell(nodeECMLocXAddr,nodeECMLocYAddr,maxMembrNodePerCell,numNodesECM,dt,Damp_CoefCell,peripORexcmAddr,currentActiveCellCount, cellGrowthProgress, mitoticThreshold, cellSubdomainIndx));//, isActiveECM));
}
//ChangesMadeByKT_end
