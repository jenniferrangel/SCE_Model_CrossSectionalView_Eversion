...
int main(int argc, char* argv[]) {
	...
	// Then initialize_v2_M will go to the simuDomain.cu and initially create some vector spaces for GPU values and then assigned the CPU values to GPU vectors either in SceNodes or SceCells. 
	// The next two functions below are the main functions for initialization of the code from txt and transfer them to GPU.
	...
	std::cout<<"mainPara.totalTimeSteps = "<<mainPara.totalTimeSteps<<std::endl;
	for (uint i = 0; i <= (uint) (mainPara.totalTimeSteps); i++) {
		// this if is just for output data// 
		if (i % mainPara.aniAuxVar == 0) {
			std::cout<<"i for vtk output is: "<<i<<std::endl;
