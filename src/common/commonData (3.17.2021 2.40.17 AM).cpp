...
std::vector<CVector> obtainPtsBetween_cellCenterLine(CVector& start, CVector& end,
	...
	for (uint j = 0; j < cellCenterLine.size(); j++) {
		nodeNew.x = cellCenterLine[j].x + 0.05*cellCenterLine_MirrorLength[j]*cellCenterLine_shift[j].x;
		if (0.05*cellCenterLine_MirrorLength[j] >= 0.5){
			nodeNew.x = cellCenterLine[j].x + 0.5*cellCenterLine_shift[j].x;
		}
		nodeNew.y = cellCenterLine[j].y + 0.05*cellCenterLine_MirrorLength[j]*cellCenterLine_shift[j].y;
		if (0.05*cellCenterLine_MirrorLength[j] >= 0.5){
			nodeNew.y = cellCenterLine[j].y + 0.5*cellCenterLine_shift[j].y;
		}
		result.push_back(nodeNew);
