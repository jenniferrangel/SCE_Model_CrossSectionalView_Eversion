	...
	if (timeRatio == 0 && cycle <= 0){
		...
		for (int i = 0; i < allocPara_M.maxTotalNodeCount; i++){
			...
			if (cellRank >= 2 && cellRank <= 15){
				infoVecs.contractActomyo_multip[i] = 1.5;//pow(contractActomyo_multip[cellRank], 3.0);////1.0;
			...
			// else if (cellRank >= 22 && cellRank <= 42){
				...
				infoVecs.contractActomyo_multip[i] = 1.5;//pow(contractActomyo_multip[cellRank], 3.0);//0.25;//1.0;
