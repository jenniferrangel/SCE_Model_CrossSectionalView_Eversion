...
struct AddMemContractForce: public thrust::unary_function<DUiDDUiUiBDDT , CVec6> {
	...
	__host__ __device__ AddMemContractForce(uint maxNodePerCell,
			uint maxMemNodePerCell, double* locXAddr, double* locYAddr, MembraneType1* MemTypeAddr, int* MirrorIndexAddr, double* locZAddr) :
			_maxNodePerCell(maxNodePerCell), _maxMemNodePerCell(
					maxMemNodePerCell), _locXAddr(locXAddr), _locYAddr(locYAddr),_MemTypeAddr(MemTypeAddr), _MirrorIndexAddr(MirrorIndexAddr), _locZAddr(locZAddr) {
	}
	// comment prevents bad formatting issues of __host__ and __device__ in Nsight
	__device__ CVec6 operator()(const DUiDDUiUiBDDT & dUiDDUiUiBDDT ) const {
		...
		double locX,locY,locXOther, locYOther, kContractMembr_multip, kContractMembr_multip2;
		uint   index ; 
		int    index_Other ; 
		...
		if  ( _MirrorIndexAddr[index] !=-1  ) { 
			...
			kContractMembr_multip = _locZAddr[index];
			...
			kContractMembr_multip2 = _locZAddr[index_Other];
			...
			if ( (nodeDistToApical>nucDistToApical)&& (nodeDistToApicalOther>nucDistToApical) ) { 
            	calAndAddMM_ContractAdh(locX, locY, locXOther, locYOther,oriVelX, oriVelY,F_MM_C_X,F_MM_C_Y, kContractMembr_multip, kContractMembr_multip2);
