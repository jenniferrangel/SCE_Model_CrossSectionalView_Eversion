	...
	int* nodeMemMirrorIndexAddr = thrust::raw_pointer_cast(
			...
			thrust::make_zip_iterator(
					thrust::make_tuple(
							    ...
							    nodes->getInfoVecs().nodeContractEnergyT.begin(),
							    nodes->getInfoVecs().basalContractPair.begin())),
			AddMemContractForce(maxAllNodePerCell, maxMemNodePerCell, nodeLocXAddr,nodeLocYAddr, nodeLocZAddr, nodeTypeAddr,nodeMemMirrorIndexAddr));
