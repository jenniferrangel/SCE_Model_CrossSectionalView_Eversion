...
struct AddMembrForce: public thrust::unary_function<TensionData, CVec10> {
	...
	double* _apicalLocX;
	double* _apicalLocY;
	MembraneType1* _memNodeType1;

	// comment prevents bad formatting issues of __host__ and __device__ in Nsight
	__host__ __device__ AddMembrForce(uint bdryCount, uint maxNodePerCell,
			double* locXAddr, double* locYAddr, bool* isActiveAddr, int* adhereIndexAddr,
			double *actinLevelAddr, double mitoticCri, double minY, double maxY, bool* isEnteringMitotic, 
			double* individualCellHeight, double* memContractProgress, /*double* distFromNucleus_normal_apical,*/ double* apicalLocX, double* apicalLocY, MembraneType1* memNodeType1) :
			_bdryCount(bdryCount), _maxNodePerCell(maxNodePerCell), _locXAddr(
					...
					_isEnteringMitotic(isEnteringMitotic), _individualCellHeight(individualCellHeight),
					_memContractProgress(memContractProgress),
					/*_distFromNucleus_normal_apical(distFromNucleus_normal_apical),*/
					_apicalLocX(apicalLocX), _apicalLocY(apicalLocY), _memNodeType1(memNodeType1) {
	}
	// comment prevents bad formatting issues of __host__ and __device__ in Nsight
	__device__ CVec10 operator()(const TensionData &tData) const {
		...
		if (_isActiveAddr[index] == false || nodeRank >= activeMembrCount) {
			...
			if (_isActiveAddr[index_left]) {
				...
				double forceVal;
				if (cellRank == 0 || cellRank == 1 || cellRank ==2 || cellRank == 3 || (cellRank > 60 && cellRank < 86)){//_isEnteringMitotic[cellRank] == false){
					forceVal = calMembrForce_Actin(lenLeft,kAvgLeft); // Ali & June 30th
				}
				else{
					...
					//_individualCellHeight[cellRank]*_distFromNucleus_normal_apical[cellRank];
					if (_memNodeType1[index] == lateralA && dist >= adhesionDist){
						forceVal = calMembrForce_ActinMitotic(lenLeft, kAvgLeft);
					}
					else if (_memNodeType1[index] == lateralB && dist >= adhesionDist){
						forceVal = calMembrForce_ActinMitotic(lenLeft, kAvgLeft);
					}
					else if (_memNodeType1[index] == basal1 && dist >= adhesionDist){
						forceVal = calMembrForce_ActinMitotic(lenLeft, kAvgLeft);
					}
					else{
			...
			if (_isActiveAddr[index_right]) {
				...
				double forceVal;
				if (cellRank == 0 || cellRank == 1 || cellRank ==2 || cellRank == 3 || (cellRank > 60 && cellRank < 86)){//_isEnteringMitotic[cellRank] == false){
					forceVal = calMembrForce_Actin(lenRight,kAvgRight); // Ali & June 30th
				}
				else{
					double dist = sqrt((_apicalLocX[cellRank] - locX)*(_apicalLocX[cellRank] - locX) + (_apicalLocY[cellRank] - locY)*(_apicalLocY[cellRank] - locY));
					double adhesionDist = _individualCellHeight[cellRank] - _memContractProgress[cellRank];
					// _individualCellHeight[cellRank]*_distFromNucleus_normal_apical[cellRank];
					if (_memNodeType1[index] == lateralA && dist >= adhesionDist){
						forceVal = calMembrForce_ActinMitotic(lenRight, kAvgRight);
					}
					else if (_memNodeType1[index] == lateralB && dist >= adhesionDist){
						forceVal = calMembrForce_ActinMitotic(lenRight, kAvgRight);
					}
...
struct AddMemContractForce_tmp2: public thrust::unary_function<DUiDDUiUiBDDT , CVec6> {
	...
	__host__ __device__ AddMemContractForce_tmp2(uint maxNodePerCell, uint maxMemNodePerCell, double* locXAddr, double* locYAddr, MembraneType1* MemTypeAddr, int* MirrorIndexAddr, 
										double timeRatio,  double* contractActomyo_multip, double* contractActomyo_multip_apical, double distFromNucleus_max, double distFromNucleus_min, double* distFromNucleus_normal, double* distFromNucleus_normal_apical,
										double percentage_before_timeRatio_Crit_Division_scaling, bool* isEnteringMitotic, double* contractileSpringGrowthProgress, double* individualCellHeight,
										double* distFromBasalLoc, double* distFromApicalLoc, double mitoRndActomyoStrengthScaling) :
			_maxNodePerCell(maxNodePerCell), _maxMemNodePerCell(
					maxMemNodePerCell), _locXAddr(locXAddr), _locYAddr(locYAddr),_MemTypeAddr(MemTypeAddr), _MirrorIndexAddr(MirrorIndexAddr), 
										_timeRatio(timeRatio), _contractActomyo_multip(contractActomyo_multip),_contractActomyo_multip_apical(contractActomyo_multip_apical),
										_distFromNucleus_max(distFromNucleus_max), _distFromNucleus_min(distFromNucleus_min), _distFromNucleus_normal(distFromNucleus_normal), _distFromNucleus_normal_apical(distFromNucleus_normal_apical),
	...
	__device__ CVec6 operator()(const DUiDDUiUiBDDT & dUiDDUiUiBDDT ) const {
		...
		if (_isEnteringMitotic[cellRank] == true){
			...
			// 	DistFromNucleus = _distFromNucleus_max;
			// }
			distMaxBasalContrSpringToApical = -9999.9;//_individualCellHeight[cellRank] - _contractileSpringGrowthProgress[cellRank];
			distMinApicalContrSpringToApical = 0.0;
			// if (distMaxBasalContrSpringToApical <= distMaxBasalContrSpringToApical_shortest_mitotic){
			if (distMaxBasalContrSpringToApical <= 0.25*distMaxBasalContrSpringToApical_shortest_mitotic){
				distMaxBasalContrSpringToApical = -9999.9;//distMaxBasalContrSpringToApical_shortest_mitotic;
			}
		}
		else{
...
struct CellInfoVecs {
	...
	thrust::device_vector<double> memContractProgress;
...
// this counting iterator is used for all these arrays indicating the begining.
	...
	void applyMembContraction2(double timeRatio, double timeRatio_Crit_actomyo, double timeRatio_Crit_Division, double distFromNucleus_max, double distFromNucleus_min, double percentage_before_timeRatio_Crit_Division_scaling, double mitoRndActomyoStrengthScaling) ; 
