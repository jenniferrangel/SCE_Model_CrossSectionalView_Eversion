	...
	if (timeRatio == 0 && cycle <= 0){
		...
		for (int i = 0; i < allocPara_M.maxTotalNodeCount; i++){
			...
			// else if (cellRank >= 22 && cellRank <= 42){
				...
				infoVecs.contractActomyo_multip[i] = 0.25;//pow(contractActomyo_multip[cellRank], 3.0);//0.25;//1.0;
				infoVecs.contractActomyo_multip_apical[i] = 1.25;
...
void SceNodes::applyMembrAdh_M() {
	...
	uint maxNodePerCell = allocPara_M.maxAllNodePerCell;
	bool* isEnteringMitotic = thrust::raw_pointer_cast(&(cellsSceNodes->getCellInfoVecs().isEnteringMitotic[0]));
	double* individualCellHeight = thrust::raw_pointer_cast(&(cellsSceNodes->getCellInfoVecs().individualCellHeight[0]));
	//double* distFromNucleus_normal_apical = thrust::raw_pointer_cast(&(cellsSceNodes->getCellInfoVecs().distFromNucleus_normal_apical[0]));
	double* memContractProgress = thrust::raw_pointer_cast(&(cellsSceNodes->getCellInfoVecs().memContractProgress[0]));
	double* apicalLocX = thrust::raw_pointer_cast(&(cellsSceNodes->getCellInfoVecs().apicalLocX[0]));
	double* apicalLocY = thrust::raw_pointer_cast(&(cellsSceNodes->getCellInfoVecs().apicalLocY[0]));
	...
	thrust::transform(
			...
			ApplyAdh(nodeLocXAddress, nodeLocYAddress, nodeGrowProAddr,nodeAdhAddr,
						nodedppLevelAddr,isApicalAdhPresent, maxNodePerCell, isEnteringMitotic, 
						individualCellHeight, memContractProgress,/*distFromNucleus_normal_apical,*/ apicalLocX, apicalLocY));
